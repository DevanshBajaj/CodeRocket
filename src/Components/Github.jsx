  ...
  return (
    <div className={styles.Container}>
      ...
      <Row justify="center" align="center" css={{ flexDirection: "row", gap: "2rem", py: "1rem" }}>
        <Input
          labelPlaceholder="githubUsername"
          type="text"
          name="githubUsername"
          id="githubUsername"
      ...
      {data && (
        <>
          <div className={styles.profile}>
            ...
            <Text size="1.2rem">Following: {data.user.following.totalCount}</Text>
            <Text size="1.2rem">Followers: {data.user.followers.totalCount}</Text>
          </div>
            ...
            {data?.user.repositories?.edges.map((github, idx) => {
              ...
              return (
                <Card key={idx}>
                  ...
                  <Text size="1.2rem">Description: {github.node.description ? (<span>{github.node.description}</span>) : (<span>N/A</span>)}</Text>
                  <div className={styles.skills}>
                    <Text size="1.2rem">Languages:</Text>
                    {github.node.languages.edges.map((lang, i) => (
                      <Text size="1.2rem" key={i} css={{ color: `${lang.node.color}`, }}>
                        {lang.node.name}
                      </Text>
                    ))}
                  </div>
